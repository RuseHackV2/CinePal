...
if(!$_SESSION['file']) die('No file selected.');
...
function generateWord($wordlist){
	...
	if(count($wordlist) == count($_SESSION['words_asked']))
	{
		...
		echo('You\'ve seen all the words! <form><button type="button" onclick="window.location=\'index.php\'">Start new session </button></form>');
		...
		Word '.count($_SESSION['words_asked']).' of '.count($wordlist).'.<form><button type="button" onclick="window.location=\''.$_SERVER['PHP_SELF'].'\'">Next word (no answer!)</button><button type="button" onclick="window.location=\'index.php\'">Start new session </button></form>
		');
	}
}
